...
import {
    ...
    IMonthReportData,
    insertPermitReportMonthDataORCL, IMonthReport,
...
export const handleConfirmZeroRemainingData = async (req: Request) => {
    ...
    if (digits === '1' && clientNum) {
        ...
        const x = permitMonthSelections.map((e) => {
            const y = e.data.map((f) => {
                return insertPermitReportMonthDataORCL({ ...f, quantity: '0' }, clientNum, "PHONE");
            });
            Promise.all(y);
        });
...
export const handlePermitMonthData = async (req: Request) => {
    ...
    const permitMonth = (req.session as any).permitMonth as IMonthReport;
    const permitReportId = (req.session as any).permitReportId;
    const newData = { ...permitMonth.data.find((e) => e.permitReportId === permitReportId), quantity: digits } as IMonthReportData;
...
export const handleConfirmPermitMonthData = async (req: Request) => {
    ...
    if (digits === '1') {
        ...
        (req.session as any).permitReportId = newData.length > 0 ? newData[0].permitReportId: (req.session as any).permitReportId;
